...
async function handleDealCreate(dealId, requestId) {
  ...
  if (shouldCreateOrder) {
    try {
      ...
      const orderType = categoryId === '4' ? 'CATALOG' : 'REGULAR';
      console.log(JSON.stringify({
        event: 'UNIFIED_ORDER_CREATION_START',
        requestId,
        dealId,
      ...
      if (orderType === 'CATALOG') {
        // Category 4: Use Brand/Model/Size UF fields
        items = await resolveCatalogOrderItems(dealId, dealData, requestId);
        ...
        if (items.length === 0) {
          console.log(JSON.stringify({
            ...
            eventType: 'CREATE',
            categoryId,
            message: 'Missing Brand/Model/Size or variant not found',
            timestamp: new Date().toISOString()
          }));
          // Returns early - no order to create in catalog mode without fields
          return {
            success: true,
            triggerMatch: false,
            skip_reason: 'no_catalog_items'
          };
        }
      }
