...
export async function createBitrixProduct(productData, catalogId = 14, sectionId = 32) {
  ...
  if (description) {
    ...
    // Autosync logic: Do not set PREVIEW_TEXT on creation, it might conflict
  }
  ...
  try {
    ...
    if (response.result) {
      ...
      // This ensures description is saved even if crm.product.add ignores it
      if (description) {
        try {
          console.log(`[BITRIX PRODUCTS] 🔄 Forcing description update for new product ${productId}...`);
          ...
          console.warn(`[BITRIX PRODUCTS] ⚠️ Failed to force description update:`, descErr);
        }
      }
